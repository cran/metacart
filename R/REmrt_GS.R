...
REmrt_GS_ <- function(mf, maxL, minbucket, minsplit, cp, lookahead){
  ...
  res.Qb = 0
  res.tau2 = (sum(y^2/vi) - (sum(y/vi))^2/sum(1/vi) - length(y) + 
                1)/(sum(1/vi) - sum(1/vi^2)/sum(1/vi))
  res.split = NA
  res.mod = NA
  res.pleaf = NA
  delta.Q <- Inf
  if (lookahead) {
    first.splits <- lapply(mods, function(x) make_first_split(x, 
                                                              y, minbucket))
    second.splits <- lapply(1:nmod, function(x) find_second_split(mods[, 
    ...
    if (first.splits[[x1.inx]]$is.num) {
      ...
      #msplit1 <- paste(mods.names[x1.inx], "<=", round(cstar1, 2), collapse = " ")
    }
    else {
      cstar1 <- names(first.splits[[x1.inx]]$rank[first.splits[[x1.inx]]$rank <= 
                                                    second.splits[[x2.inx]]$split1[2]])
      msplit1 <- paste(mods.names[x1.inx], "=", paste(cstar1, 
                                                      collapse = "/"), collapse = " ")
    }
    ...
    if (is.null(second.splits[[x2.inx]]$split2$rank)) {
      ...
      node2[pleaf.inx] <- ifelse(mods[pleaf.inx, x2.inx] <= 
                                   cstar2, 4, 5)
      msplit2 <- paste(mods.names[x2.inx], "<=", cstar2, collapse = " ")
      #msplit2 <- paste(mods.names[x2.inx], "<=", round(cstar2, 2), collapse = " ")
    }
    else {
      ...
      node2[pleaf.inx] <- ifelse(mods[pleaf.inx, x2.inx] %in% 
                                   cstar2, 4, 5)
      msplit2 <- paste(mods.names[x2.inx], "=", paste(cstar2, 
                                                      collapse = "/"), collapse = " ")
    }
        ...
        for (k in 1:nmod) {
          ...
          if (is.numeric(xk)) {
            ...
            if (Dev.new > Dev) {
              ...
              # msplit <- paste(mods.names[k], "<=", round(c.star, 
              #                                            2), collapse = " ")
              TQb = temp[2]
              Ttau2 = temp[3]
              Tsplit = msplit
            ...
            xk.rank <- rank(tapply(y[pleaf.inx], xk, 
                                   mean))
            xk.ordinal <- xk.rank[as.character(xk)]
            temp <- re.cutoff_cpp(y, vi, xk.ordinal, 
                                  pleaf.inx, cnode, minbucket)
            if (is.null(temp)) {
              Dev.new <- -Inf
            }
            else {
            ...
            if (Dev.new > Dev) {
              ...
              msplit <- paste(mods.names[k], "=", paste(c.star, 
                                                        collapse = "/"), collapse = " ")
              TQb = temp[2]
              Ttau2 = temp[3]
              Tsplit = msplit
              ...
              Tpleaf = as.numeric(pl)
              new.node <- cnode
              new.node[pleaf.inx] <- ifelse(xk.ordinal <= 
                                              temp[1], 2 * i, 2 * i + 1)
            }
          }
        }
    ...
    for (pl in nodes) {
      ...
      for (k in 1:nmod) {
        ...
        if (is.numeric(xk)) {
          ...
          xk.ordinal <- xk.rank[as.character(xk)]
          temp <- re.cutoff_cpp(y, vi, xk.ordinal, pleaf.inx, 
                                cnode, minbucket)
          if (is.null(temp)) {
            Dev.new <- -Inf
          }
