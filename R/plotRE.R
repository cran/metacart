...
plot.REmrt <- function(x, ...){
  if (length(x$n) < 2) {stop("no tree was detected")}
    ...
    d <- x$data
    d$term.node <- sapply(d$term.node, as.factor)

    # all terminal nodes
    term <- nodes[is.na(nodes$split),]
    # with ascending x
    term <- term[ordered(term$x.new),]
    d$yi <- model.response(x$data)



    p <- ggplot(d, aes(term.node, yi)) +
      geom_boxplot(
        position = position_dodge(width = 0),
        width = 0.5
        # alpha = 0.5
        # TODO: change lower/middle/upper
        #   aes(
        #   lower = res$ci.lb,
        #   middle = res$g,
        #   upper = res$ci.ub
      ...
      scale_x_discrete(limits = as.factor(term$leaf))


    p <- p + geom_hline(
      yintercept = c(min(d$yi), max(d$yi)),
      linetype = "dashed"
    )
